...
dPLC_particle_dt_func = @(act_cPKC_particle, PLC_particle, GqGPCR_particle) PLC_particle * get_trans_matrix(act_cPKC_particle, GqGPCR_particle, theta);
end

% compute the transition probability matrix
function trans_matrix = get_trans_matrix(act_cPKC_particle, GqGPCR_particle, theta)
b_PLC_produce = theta('b_PLC_produce');
b_PLC_degrade = theta('b_PLC_degrade');
act_cPKC = mean(act_cPKC_particle(:,2));
GqGPCR = mean(GqGPCR_particle(:,2));
other_degrade = theta('b_PLC_degrade2');
activate = b_PLC_produce * GqGPCR; % activated by receptors
